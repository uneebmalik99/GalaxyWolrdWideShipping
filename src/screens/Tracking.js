...
const Tracking = ({ navigation }) => {
  ...
  const searchingApi =()=>{
    ...
    var url = '';
        url = AppUrlCollection.VEHICLE_CONTAINER + 'search_str=' + search + '&page=1';
...
<View style={{ width: '30%',justifyContent:'center', height: 80 }}
       ...
       <View style={{width:'64%', height:'100%',justifyContent:'center'}}>
         <Text style={{color:'white'}}>{item.year} {item.make} {item.model}</Text>
         <Text style={{color:'white'}}>{item.location == 2 ?  'GA': item.location == 4 ? 'TX': item.location == 1 ? 'LA' : item.location == 8 ?'TORONTO': item.location == 9 ?'MONTREAL': item.location == 12 ? 'GALGARY':item.location == 10 ? 'HALIFAX' : 'NY' } | {item.lot_number}</Text>
...
<Text style={{alignSelf:'center',color:'white',fontWeight:'bold', fontSize:20}}>Car Tracking </Text>
</View>
